# HANDOFF — Cyclades : VOYAGE COMPLET 5/5 EN PROD ✅ (Syros · Sifnos · Folegandros · Amorgos · Santorin) — le cercle est fermé (MAJ 2026-06-15)

> **MAJ 2026-06-15 (5) — SANTORIN DÉPLOYÉ, VOYAGE CYCLADES COMPLET 5/5.** Santorin ch.5 (le final) mergé→main (`be04938`), live sur /cyclades/santorin/ (vérifié curl 200 + scènes : le vin de la cendre/Assyrtiko, fava+tomatokeftedes, dédale d'Emporio, poisson d'Ammoudi, bain sur le sable noir de Vlychada). Angle anti-carte-postale tenu : terroir volcanique + bouffe + eau en vedette, Oia/caldeira en backdrop. 26 POIs, 7 dishes, 7 images CC scellées (**41/41 MATCH vision cyclades, 0 mismatch**), `npm run audit` VERT, déploy GH Actions ✓. tilesIntro container = « Les cinq îles sont écrites à fond ». **Recherche salvagée des transcripts d'agents** (le fan-out s'était coupé à la compaction) → dossier `.research/santorin-dossier.md`.
> - **Corrections post-revue Martin (mêmes merge) :** (1) `amo-kastani` (Amorgos) — le lien Google Maps flou tombait sur un lieu-dit « Kastanis » près d'un camping → coord resserrée sur la Chora (OSM), lien Maps **ancré sur coords**, TripAdvisor ajouté, blurb enrichi (taverne depuis 1962). **Audit reverse-géocode des 109 épingles cyclades = 0 mauvaise position.** (2) `san-emporio-elefsina` **fermé** → remplacé par **Exi Castle Suites** (kasteli vénitien d'Emporio, ~76 $, site officiel + 314 avis Booking + TripAdvisor).
> - **PROCHAINE ÉTAPE (notée, pas commencée) :** revoir l'**Andalousie** en prod sous le même angle oomph/anti-carte-postale (bouffe+eau en vedette, style imagerie `style-imagerie-voyage`). Machine de voyage prouvée from-scratch sur 2 voyages (Andalousie partielle + Cyclades complet).
>
> **MAJ 2026-06-15 (4) — AMORGOS DÉPLOYÉ, 4/5 EN PROD.** Amorgos ch.4 mergé→main (`48ab7a4`), live sur /cyclades/amorgos/ (vérifié curl 200 + scènes : monastère Hozoviotissa, patatato, épave Kalotaritissa). Même pipeline. Reste **SANTORIN (ch.5, le final — la catastrophe comme terroir : Assyrtiko/fava/tomataki sur la cendre volcanique)** : recherche PAS encore lancée, build PAS commencé. Reprise à froid : « fais Santorin » → fan-out recherche Sonnet (spine déjà dans cyclades-spine.md) → images CC Santorin-vraies → rebuild → audit → iOS → verdict → merge. (Note : Santorin = l'île la plus photographiée → Commons riche, mais ATTENTION au piège carte-postale : garder bouffe+terroir volcanique en vedette, pas juste les dômes bleus de la caldeira.)
>
...
## ⚠️ STATUT — 4 chapitres DÉPLOYÉS (Syros, Sifnos, Folegandros, Amorgos) ; reste SANTORIN (ch.5, le final)
- **Syros + Sifnos + Folegandros mergés→`main`, LIVE** sur https://chartrandapps.ca/cyclades/. L'Andalousie reste en prod aussi. **Chaque nouvelle île = NOUVELLE branche `feat/cyclades-<île>` depuis `main`** (`main` auto-deploie à chaque push). Amorgos = ch.4 LIVE (en 2 bases). **Reste Santorin = ch.5, le final** (recherche pas encore lancée ; branche `feat/cyclades-santorin` depuis main).
